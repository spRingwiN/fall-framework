...
import com.eric.fall.web.controller.ApiController;
import com.eric.fall.web.controller.ControllerConfiguration;
...
public class DispatcherServletTest {
    ...
    void getDownload() throws ServletException, IOException {
        ...
        this.dispatcherServlet.service(req, resp);
        assertEquals(200, resp.getStatus());
        assertArrayEquals("AAAAAAAA".getBytes(StandardCharsets.UTF_8), resp.getContentAsByteArray());
    }
    ...
    void getApiDownload() throws ServletException, IOException {
        ...
        /**
         * JSON序列化要求
         * 二进制数据处理: JSON格式无法直接表示二进制数据（byte[]）
         * 文本格式限制: JSON是纯文本格式，需要将二进制内容转换为文本表示
         * 字节数组转换
         * byte[] 序列化: 在JSON序列化过程中，字节数组会自动转换为Base64编码字符串
         * 标准编码方式: Base64是将二进制数据编码为ASCII文本的标准方法
         */
    ...
    void getLogin() throws ServletException, IOException {
        ...
        assertEquals("/signin", resp.getHeader("Location"));
    ...
    void getProduct() throws ServletException, IOException {
        ...
        assertTrue(resp.getContentAsString().contains("<a href=\"/product/123\">Fall Software</a>"));
    }
    ...
    void postSignin() throws ServletException, IOException {
        ...
        this.dispatcherServlet.service(req, resp);
        assertEquals(302, resp.getStatus());
        assertEquals("/home?name=Bob", resp.getHeader("Location"));
    ...
    void postSignout() throws ServletException, IOException {
        ...
        this.dispatcherServlet.service(req, resp);
        assertEquals(302, resp.getStatus());
        assertEquals("/signin?name=Bob", resp.getHeader("Location"));
        assertEquals("/signin?name=Bob", resp.getRedirectedUrl());
        assertTrue(req.getSession().getAttribute("signout").equals(Boolean.TRUE));
    }
    ...
    void init() throws ServletException {
        ...
        var applicationContext = new AnnotationConfigApplication(ControllerConfiguration.class, propertyResolver);
    ...
    PropertyResolver createPropertyResolver() {
        ...
        ps.put("fall.web.favicon-path", "/icon/favicon.ico");
        ps.put("fall.web.freemarker.template-path", "/WEB-INF/templates");
