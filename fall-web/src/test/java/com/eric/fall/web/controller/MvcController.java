...
import com.eric.fall.web.ModelAndView;
...
@ResponseBody
byte[] download(@PathVariable("file") String file,
...
ModelAndView product(@PathVariable("id") long id, @RequestParam("name") String name) {
    return new ModelAndView("/product.html", Map.of("name", name, "product", Map.of("id", id,
            "name", "Fall Software")));
}
