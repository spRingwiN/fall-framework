...
public class ApiController {
    ...
    FileObj download(@PathVariable("file") String file,
    ...
    void downloadPart(@RequestParam("file") String file,
                      @RequestParam("time") Float downloadTime,
                      @RequestParam("md5") String md5,
                      @RequestParam("length") int length,
                      @RequestParam("hasChecksum") boolean checksum,
    ...
    void register(@RequestBody SigninObj signinObj, HttpServletResponse resp) throws IOException {
        resp.setContentType("application/json");
        PrintWriter pw = resp.getWriter();
        pw.write(String.format("[\"%s\",true,12345]", signinObj.name));
        pw.flush();
    }
