package com.eric.fall.context;

import com.eric.fall.annotation.*;
import com.eric.fall.exception.*;
import com.eric.fall.io.PropertyResolver;
import com.eric.fall.io.ResourceResolver;
...
public class AnnotationConfigApplication implements ConfigurableApplicationContext {
    ...
    private Object getProxiedInstance(BeanDefinition def) {
        ...
        List<BeanPostProcessor> reversedProcessors = new ArrayList<>(this.beanPostProcessors);
        ...
        for (BeanPostProcessor beanPostProcessor : reversedProcessors) {
            ...
            if (restoredInstance != beanInstance) {
                logger.atDebug().log("BeanPostProcessor {} specified injection from {} to {}", beanPostProcessor.getClass().getName(),
    ...
    /**
     * 创建一个Bean，但不进行字段和方法级别的注入。如果创建的Bean，
     * 不是Configuration，则在构造方法中注入的依赖Bean会自动创建
     * @param def
     * @return
     */
    public Object createBeanAsEarlySingleton(BeanDefinition def) {
        ...
        for (int i = 0; i < parameters.length; i++) {
            ...
            // @Configuration 类型的Bean是工厂，不允许使用@Autowired创建
            final boolean isConfiguration = isConfigurationDefinition(def);
            if (isConfiguration && autowired != null) {
                throw new BeanCreationException(
                        String.format("Can not specify @Autowired when create @Cofiguration bean '%s': %s", def.getName(), def.getBeanClass().getName())
                );
            }
            ...
            if (isBeanPostProcessor && autowired != null) {
                throw new BeanDefinitionException(
                        String.format("Can not specify @Autowired when create BeanPostProcessor '%s': '%s'.", def.getName(), def.getBeanClass().getName())
                );
            }
            // 参数需要@Value 或者@Autowired 两者之一：
            if (value != null && autowired != null) {
                throw new BeanCreationException(
                        String.format("Can not specify both @Autowired and @Value when create bean '%s': %s", def.getName(), def.getBeanClass().getName())
                );
            }
            if (value == null && autowired == null) {
                throw new BeanCreationException(
        ...
        if (def.getFactoryName() == null) {
            ...
            try {
                ...
                throw new BeanCreationException(
                        String.format("Exception when create bean '%s': %s", def.getName(), def.getBeanClass().getName()), e);
            }
        } else {
            // 用 @Bean 方法创建
        ...
        for (BeanPostProcessor processor : beanPostProcessors) {
            ...
            if (processed == null) {
                throw new BeanCreationException(
                        String.format("PostBeanProcessor '%s' returns null when process bean '%s': %s", processor.getClass().getName(), def.getName(), def.getBeanClass().getName())
                );
            }
            // 如果一个BeanPostProcessor替换了原始的Bean，则更新Bean的引用:
            ...
            if (component != null) {
                ...
                if (configuration != null) {
                    if (BeanPostProcessor.class.isAssignableFrom(clazz)) {
                        throw new BeanDefinitionException(
                                String.format("@Configuration class '%s' can not be BeanPostProcessor", clazz.getName())
                        );
                    }
    ...
    void tryInjectProperties(BeanDefinition def, Class<?> clazz, Object bean, AccessibleObject acc) throws ReflectiveOperationException {
        ...
        if (acc instanceof Method m) {
            ...
            if (m.getParameters().length != 1) {
                throw new BeanDefinitionException(
                        String.format("Can not inject a non-setter method %s for bean '%s': %s", m.getName(), def.getName(), def.getBeanClass().getName())
                );
            }
        ...
        if (value != null && autowired != null) {
            throw new BeanCreationException(
                    String.format("Can not specify both @Autowired and @Value when inject %s.%s for bean '%s': %s",
    ...
    void checkFieldOrMethod(Member m) {
        ...
        if (Modifier.isStatic(mod)) {
            throw new BeanDefinitionException("Can not inject static field: " + m);
        }
    ...
    /**
     * 通过Name 和 Type查找Bean， 不存在抛出NoSuchBeanDefinitionException,
     * 存在但与Type不匹配时抛出BeanNotOfRequiredTypeException
     * @return
     * @param <T>
     */
    @Override
    public <T> T getBean(String name, Class<T> requiredType) {
        ...
        if (t == null) {
            throw new NoSuchBeanDefinitionException(String.format("No bean defined with name '%s' and type '%s'", name, requiredType.getName()));
        }
        return t;
    }
    ...
    /**
     * 通过Type查找Bean，不存在抛出NoSuchBeanDefinitionException,
     * 存在多个但缺少唯一的@Primary时抛出NoUniqueBeanDefinitionException
     * @param requiredType
     * @return
     * @param <T>
     */
    ...
    // findXxx 与 getXxx类似，但不存在时返回null
    ...
    protected <T> List<T> findBeans(Class<T> requiredType) {
        return findBeanDefinitions(requiredType)
                .stream().map(def -> (T) def.getRequiredInstance()).collect(Collectors.toList());
